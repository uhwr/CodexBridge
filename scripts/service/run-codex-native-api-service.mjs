...
const restartMs = Math.max(0, Number.parseFloat(args.restartSec ?? process.env.CODEXBRIDGE_CODEX_SERVICE_RESTART_SEC ?? '2') * 1000);
...
async function runServe() {
  ...
  writeLine('stdout', `[codexbridge-codex-service] starting: ${process.execPath} ${serveArgs.join(' ')}`);
  writeLine('stdout', `[codexbridge-codex-service] env HOME=${process.env.HOME ?? ''}`);
  writeLine('stdout', `[codexbridge-codex-service] env USERPROFILE=${process.env.USERPROFILE ?? ''}`);
  writeLine('stdout', `[codexbridge-codex-service] env CODEX_HOME=${process.env.CODEX_HOME ?? ''}`);
  writeLine('stdout', `[codexbridge-codex-service] env CODEX_REAL_BIN=${process.env.CODEX_REAL_BIN ?? ''}`);
  writeLine('stdout', `[codexbridge-codex-service] env CODEX_DEFAULT_PROVIDER_PROFILE_ID=${process.env.CODEX_DEFAULT_PROVIDER_PROFILE_ID ?? ''}`);
  writeLine('stdout', `[codexbridge-codex-service] env CODEX_NATIVE_API_PROVIDER_PROFILE_ID=${process.env.CODEX_NATIVE_API_PROVIDER_PROFILE_ID ?? ''}`);
  ...
  return new Promise((resolve) => {
    child.once('error', (error) => {
      writeLine('stderr', `[codexbridge-codex-service] child spawn failed: ${error.stack || error.message}`);
      resolve(1);
    });
    child.once('exit', (code, signal) => {
      writeLine('stderr', `[codexbridge-codex-service] child exited code=${code ?? ''} signal=${signal ?? ''}`);
      child = null;
      resolve(code);
    });
  });
...
function stop(signal) {
  ...
  stopping = true;
  writeLine('stderr', `[codexbridge-codex-service] stopping on ${signal}`);
